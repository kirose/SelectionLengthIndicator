...
public abstract class AbstractTextEditor extends EditorPart implements ITextEditor, IReusableEditor, ITextEditorExtension, ITextEditorExtension2, ITextEditorExtension3, ITextEditorExtension4, ITextEditorExtension5, ITextEditorExtension6, INavigationLocationProvider, ISaveablesSource, IPersistableEditor {
	...
	// MASB Marco A. Salazar
	/** The pattern used to show the position label in the status line. */
	private final String fPositionLabelPattern= EditorMessages.Editor_statusline_position_length_selection_pattern;//EditorMessages.Editor_statusline_position_pattern;
	...
	// MASB Marco A. Salazar
	/** The position label value of the length of the selection. */
	private final PositionLabelValue fLengthSelectionLabel= new PositionLabelValue();
	// MASB Marco A. Salazar
	/** The arguments for the position label pattern. */
	private final Object[] fPositionLabelPatternArguments= new Object[] { fLineLabel, fColumnLabel, fLengthSelectionLabel };
	/**
...
//			getSite().getWorkbenchWindow().getWorkbench().getProgressService().run(false, true, runnable);
	...
	public void createPartControl(Composite parent) {
		...
		/**
		 * MASB Marco A. Salazar
		 * Add listener to selection provider to be notified when selection changes
		 */
		fSourceViewer.getSelectionProvider().addSelectionChangedListener(new org.eclipse.jface.viewers.ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				if (event.getSelection() != null && event.getSelection() instanceof ITextSelection) {
					fLengthSelectionLabel.fValue = ((ITextSelection) event.getSelection()).getLength();
